...
const factors_length = [1e10,66845871226706e-25,100,.049709695378987,10,.54680664916885,3.2808398950131,.0049709695378987,39.370078740157,.001,.00020712373074577,10570008340246e-29,1,.00062137119223733,1e3,1e6,1e9,.00053995680345572,32407792896393e-30,.19883878151595,1.0936132983377];
...
if (whatLang() == "el") {
  ...
  cb_length = ["άνγκστρομ", "αστρονομική μονάδα (au)", "εκατοστά (cm)", "chain", "δεκατόμετρα (dm)", "οργιά (fathom)", "πόδια (ft)", "φέρλονγκ", "ίντσες (in)", "χιλιόμετρα (km)", "λεύγα(league)", "έτη φωτός", "μέτρα (m)", "μίλια (mi)", "χιλιοστά (mm)", "μικρόμετρα (&mu;)", "νανόμετρα (nm)", "ναυτικά μίλια", "παρσέκ (parsec)", "rod", "γιάρδες (yd)"];
}else {
  ...
  cb_length = ["angstrom", "astronomical unit (au)", "centimeter (cm)", "chain", "decimeter (dm)", "fathom", "foot (ft)", "furlong", "inch (in)", "kilometer (km)", "league", "light year", "meter (m)", "mile (mi)", "millimeter (mm)", "micron (&mu;)", "nanometer (nm)", "nautical mile", "parsec", "rod", "yard (yd)"];
...
document.getElementById('app12_form').addEventListener('submit', app12_calculateResults);


function app12_calculateResults(e) {

  convert_length();



function fix_length(e){if(!isFinite(e))return"";if(0==e)return"0";if(st=""+e,epos=st.indexOf("E"),-1==epos&&(epos=st.indexOf("e")),sdigi=Math.log(Math.abs(e))/Math.LN10,sdigif=Math.floor(sdigi),-1==epos)return adjust=Math.pow(10,sdigif-12),faqs=Math.round(e/adjust),norst=""+faqs,sdigif<12?(adjust2=Math.pow(10,12-sdigif),faqs/adjust2):faqs*adjust;for(zo=e*Math.pow(10,12-sdigif),szo=String(Math.round(zo)),inse=-1,"-"==szo.charAt(0)?inse=2:inse=1,rest=szo.substring(inse,szo.length),i=rest.length-1;0<=i&&"0"==rest.charAt(i);)i--;return rest=rest.substring(0,i+1),rou=szo.substring(0,inse),0<rest.length&&(rou+="."+rest),sdigif<0?sa=rou+"E":sa=rou+"E+",snow=sa+sdigif,vanow=Math.abs(parseFloat(snow)),faqsvab=Math.abs(e),0<=sdigif?vanow>5*faqsvab?snow=sa+String(sdigif-1):vanow<faqsvab/5&&(snow=sa+String(sdigif+1)):0<=sdigif&&(vanow>5*faqsvab?snow=sa+String(sdigif+1):vanow<faqsvab/5&&(snow=sa+String(sdigif-1))),vanow=parseFloat(snow),vanow>1.1*e||vanow<.9*e?e:snow}


function convert_length() {
  let from_select_index = document.getElementById("app12_select_id_0").value;
  let to_select_index = document.getElementById("app12_select_id_1").value;
  let from_input = document.getElementById("app12_input_id_0").value;
  from_select_index = Number(from_select_index);
  to_select_index = Number(to_select_index);
  from_input = Number(from_input);

  let faqsorg = Number(factors_length[to_select_index]) / Number(factors_length[from_select_index]);
  let resfaqs = from_input * faqsorg;

  document.getElementById('app12_results_title_0').innerHTML = cb_length[from_select_index];
  document.getElementById('app12_results_title_1').innerHTML = cb_length[to_select_index];
  document.getElementById('app12_results_span_content_0').innerHTML = from_input;
  isNaN(parseFloat(resfaqs)) ?  document.getElementById('app12_results_span_content_1').innerHTML = '' :  document.getElementById('app12_results_span_content_1').innerHTML = fix_length(parseFloat(resfaqs));
  document.querySelector(".results .panel-title").innerHTML = finalResults;
  
  
  // from_select_index = $("#app12_select_id_0").val();
  // to_select_index = $("#app12_select_id_1").val();
  // from_input = $("#app12_input_id_0").val();
  // faqsorg = factors_length[to_select_index] / factors_length[from_select_index];
  // resfaqs = from_input * faqsorg;
  // $("app12_results_title_0").html(cb_length[from_select_index]);
  // $("app12_results_title_1").html(cb_length[to_select_index]);
  // isNaN(parseFloat(resfaqs)) ? $("#app12_results_span_content_1").val("") : $("#app12_results_span_content_1").val(fix_length(parseFloat(resfaqs)) + " ")
}
...
// , factors_weight = [60221366516752e13, 5e3, .022046226218488, 1e5, 100, 564.38339119329, 15432.358352941, 1e3, .019684130552221, 1, 1e9, 1e6, 9.80665, 35.27396194958, 643.01493137256, 2.2046226218488, .078736522208885, .15747304441777, .00098420652761106, .0011023113109244, .001, 32.150746568628];
...
//   if (st = "" + e,
//   epos = st.indexOf("E"),
//   -1 == epos && (epos = st.indexOf("e")),
//   sdigi = Math.log(Math.abs(e)) / Math.LN10,
//   sdigif = Math.floor(sdigi),
//   -1 == epos)
//       return adjust = Math.pow(10, sdigif - 12),
//       faqs = Math.round(e / adjust),
//       norst = "" + faqs,
//       sdigif < 12 ? (adjust2 = Math.pow(10, 12 - sdigif),
//       faqs / adjust2) : faqs * adjust;
//   for (zo = e * Math.pow(10, 12 - sdigif),
//   szo = String(Math.round(zo)),
//   inse = -1,
//   "-" == szo.charAt(0) ? inse = 2 : inse = 1,
//   rest = szo.substring(inse, szo.length),
//   i = rest.length - 1; 0 <= i && "0" == rest.charAt(i); )
//       i--;
...
// }
// var cb_area = ["acre", "are (a)", "barn (b)", "hectare (ha)", "homestead", "rood", "square centimeter (cm<sup>2</sup>)", "square foot (ft<sup>2</sup>)", "square inch (in<sup>2</sup>)", "square kilometer (km<sup>2</sup>)", "square meter (m<sup>2</sup>)", "square mile", "square millimeter (mm<sup>2</sup>)", "square rod", "square yard (yd<sup>2</sup>)", "township (twp)"]
// , factors_area = [.00024710538146717, .01, 1e28, 1e-4, 15444086341698e-19, .00098842152586866, 1e4, 10.76391041671, 1550.0031000062, 1e-6, 1, 3.8610215854245e-7, 1e6, .039536861034746, 1.1959900463011, 1.0725059959512e-8];
// function fix_area(e) {
//   if (!isFinite(e))
//       return "";
//   if (0 == e)
...
// }
// var cb_temp = ["Celsius", "Fahrenheit", "Rankine", "Reaumur", "kelvin"];
...
//   if (0 == t ? e += 273.15 : 1 == t ? e = (e - 32) / 1.8 + 273.15 : 2 == t ? e /= 1.8 : 3 == t && (e = 1.25 * e + 273.15),
//   e < 0)
//       return "Below Absolute Zero";
//   if (0 == a ? e -= 273.15 : 1 == a ? e = 1.8 * (e - 273.15) + 32 : 2 == a ? e *= 1.8 : 3 == a && (e = (e - 273.15) / 1.25),
...
//   $("#to_input").val(value)
// }
// var cb_force = [["attonewton", 1e19], ["centinewton", 100], ["decigram-force", 1019.716213], ["decinewton", 10], ["dekagram-force", 10.19716213], ["dekanewton", .1], ["dyne", 1e5], ["exanewton", 1e-17], ["femtonewton", 1e16], ["giganewton", 1e-8], ["gram-force", 101.9716213], ["hectonewton", .01], ["joule / meter (J/m)", 1], ["kilogram-force (kgf)", .1019716], ["kilonewton (kN)", .001], ["kilopond (kp)", .1019716], ["kip (kip)", 2248e-7], ["meganewton", 1e-5], ["megapond", .0001019716213], ["micronewton (µN)", 1e6], ["millinewton (mN)", 1e3], ["nanonewton (nN)", 1e9], ["newton (N)", 1], ["ounce-force (ozf)", 3.5969431019], ["petanewton", 1e-14], ["piconewton", 1e12], ["pond", 101.9716213], ["pound-force (lbf)", .22480894387], ["poundal (pdl)", 7.2330140801], ["sthene (sn)", .001], ["teranewton", 1e-11], ["ton-force (long)(tnf)", .00010036113566], ["ton-force (metric)(tnf)", .0001019716213], ["ton-force (short)(tnf)", .00011240447194], ["yoctonewton", 1e25], ["yottanewton", 1e-23], ["zeptonewton", 1e22], ["zettanewton", 1e-20]];
...
//   $("#to_input").val(value)
// }
// var cb_mass = [["carats (metric)", 2e-4], ["cental", 45.359237], ["Earth masses", 598e22], ["grains", 6479891e-11], ["grams", .001], ["hundredweights", 50.80234544], ["kilograms [kg]", 1], ["ounces (US &amp; UK)", .028349523125], ["ounces (troy, precious metals)", .0311034768], ["pounds [lbs] (US &amp; UK)", .45359237], ["pounds (troy, precious metals)", .3732417216], ["Solar masses", 1.989e30], ["slugs (g-pounds)", 14.593903], ["stones", 6.35029318], ["tons (UK or long)", 1016.0469088], ["tons (US or short)", 907.18474], ["tonnes", 1e3]];
...
// }
// var cb_frequency = [["1/second", 1], ["cycle/second", 1], ["degree/hour", 1 / 1296e3], ["degree/minute", 1 / 21600], ["degree/second", 1 / 360], ["gigahertz", 1e9], ["hertz", 1], ["kilohertz", 1e3], ["megahertz", 1e6], ["millihertz", .001], ["radian/hour", 1 / 22619.467], ["radian/minute", 1 / 376.99112], ["radian/second", 1 / 6.2831853], ["revolution/hour", 1 / 3600], ["revolution/minute", 1 / 60], ["revolution/second", 1], ["RPM", 1 / 60], ["terrahertz", 1e12]];
