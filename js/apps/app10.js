// common.js is a dictionary file used by passchk.js.  GPLv3 applies
//
...
if (whatLang() == "el") {
  ...
  error3 = "Συμπληρώστε το ποσό";
  error4 = "Συμπληρώστε το ποσό";
} else {
  finalResults = "Final Results";
  error1 = "The currencies you chose are the same. Choose differently!";
  error2 = "Enter the amount";
  error3 = "Enter the amount";
  error4 = "Enter the amount";
}



document.getElementById('app10_form').addEventListener('submit', app10_calculateResults);

function app10_calculateResults(e) {
  var length = document.getElementById('app10_input_id_0').value;
  var numberFirst = document.getElementById('app10_input_id_1').checked;
  var lowerFirst = document.getElementById('app10_input_id_2').checked;
  var upperFirst = document.getElementById('app10_input_id_3').checked;
  var symbolFirst = document.getElementById('app10_input_id_4').checked;
  var numberOther = document.getElementById('app10_input_id_5').checked;
  var lowerOther = document.getElementById('app10_input_id_6').checked;
  var upperOther = document.getElementById('app10_input_id_7').checked;
  var symbolOther = document.getElementById('app10_input_id_8').checked;
  ...
  if (!numberFirst && !lowerFirst && !upperFirst && !symbolFirst && !numberOther &&  !lowerOther && !upperOther && !symbolOther ) {
    showError(error2);
    return false;
  }

  if (!numberFirst && !lowerFirst && !upperFirst && !symbolFirst  ) {
    showError(error3);
    return false;
  }


  if ( !numberOther &&  !lowerOther && !upperOther && !symbolOther ) {
    showError(error4);
    return false;
  }
...
generatePassword();
function generatePassword() { // traditional passwords
  var newPass = getPassword(length, numberFirst, lowerFirst, upperFirst, symbolFirst, numberOther, lowerOther, upperOther, symbolOther);
  document.getElementById('app10_results_span_content_0').innerHTML = newPass;
  // $('.results textarea').val(newPass);
  // $('.results').slideDown('fast');
}


// function generateWordBasedPassword() {
//   var length = document.wordForm.numWords.value;
//   var numLength = document.wordForm.numNums.value;
//   var segments = [];

//   for (var c = 0; c < length; c++) {
//     segments.push(pickWord());
//   }
...
//   out = segments.join('');
//   $('.wordResults textarea').val(out);
//   $('.wordResults').slideDown('fast');
// }
...
/* -- below based on code from this site:
	http://www.javascriptsource.com/passwords/password-generator.html
*/
...
// removed "0" and "O" to avoid confusion
...
function getPassword(length, firstNumber, firstLower, firstUpper, firstOther,
